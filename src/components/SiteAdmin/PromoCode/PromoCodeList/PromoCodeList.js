...
class PromoCodeList extends React.Component {
  ...
  render() {
    const { promoCodes, promoCodes: { getPromoCodes }, loading } = this.props;
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td data-label={formatMessage(messages.status)}>
                          <select name="isEnable" onChange={(e) => this.handleChange(e, item.id, item.title, item.description, item.code, item.type, item.promoValue)} className={bt.selectInput} value={status}>
                        ...
                        <td data-label={formatMessage(messages.action)}>
                          <Link to={'/siteadmin/promo-code/edit/' + item.id} className={cx('editAlign')}>
