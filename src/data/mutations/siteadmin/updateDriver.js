    ...
    async resolve({ request }, {
        ...
        if (request.user && request.user.admin) {
            ...
            const driverStatus = await Booking.findOne({
                ...
                where: {
                    driverId: id,
                    tripStatus: { in: ['approved', 'created', 'started'] },
                },
                raw: true
            });

            const existingStatus = await User.findOne({
                attributes: ['isBan', 'userStatus'],
                where: {
                    id,
                    deletedAt: null
                },
            ...
            if (driverStatus && driverStatus.id && (existingStatus.isBan != isBan || existingStatus.userStatus != userStatus)) {
                return {
                    status: 400,
